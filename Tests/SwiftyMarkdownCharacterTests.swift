...
class SwiftyMarkdownStylingTests: SwiftyMarkdownCharacterTests {
	...
	func testThatItalicTraitsAreParsedCorrectly() {
		...
		Line break
		""", tokens: [
			Token(type: .string, inputString: "An asterisk: *\nLine break", characterStyles: [])
		])
		results = attempt(challenge)
		XCTAssertEqual(results.stringTokens.count, challenge.tokens.count)
		XCTAssertEqual(results.foundStyles, results.expectedStyles)
	...
	func offtestThatMarkdownMistakesAreHandledAppropriately() {
		...
		let md = Parser()
		XCTAssertEqual(md.attributedString(mismatchedBoldCharactersAtStart).string, "This should be bold")
		XCTAssertEqual(md.attributedString(mismatchedBoldCharactersWithin).string, "A string that should be italic")
	...
	func testThatAsterisksAndUnderscoresNotAttachedToWordsAreNotRemoved() {
		...
		let md = Parser()
		Parser.characterRules = self.defaultRules
		XCTAssertEqual(md.attributedString(backtickSpace).string, backtickSpace)
		XCTAssertEqual(md.attributedString(underscoreSpace).string, underscoreSpace)
		XCTAssertEqual(md.attributedString(asteriskFullStop).string, asteriskFullStop)
		XCTAssertEqual(md.attributedString(underscoreFullStop).string, underscoreFullStop)
		XCTAssertEqual(md.attributedString(asteriskComma).string, asteriskComma)
		XCTAssertEqual(md.attributedString(backtickComma).string, backtickComma)
		XCTAssertEqual(md.attributedString(underscoreComma).string, underscoreComma)
		XCTAssertEqual(md.attributedString(asteriskWithBold).string, "A bold word followed by an asterisk *")
		XCTAssertEqual(md.attributedString(backtickWithCode).string, "A code word followed by a backtick `")
		XCTAssertEqual(md.attributedString(underscoreWithItalic).string, "An italic word followed by an underscore _")
